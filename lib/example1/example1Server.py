...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'example1'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from example1.example1Impl import example1
impl_example1 = example1(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['example1.filter_contigs_async'] = ['example1', 'filter_contigs']
async_check_methods['example1.filter_contigs_check'] = ['example1', 'filter_contigs']
sync_methods['example1.filter_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'example1'
        ...
        self.rpc_service.add(impl_example1.filter_contigs,
                             name='example1.filter_contigs',
                             types=[dict])
        self.method_authentication['example1.filter_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "example1 but no authentication header was passed"
                            raise err
