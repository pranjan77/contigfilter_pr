package example1::example1Client;
...
sub new
{
    ...
    my $self = {
	client => example1::example1Client::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is an example1.FilterContigsParams
$return is an example1.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is an example1.workspace_name
	contigset_id has a value which is an example1.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is an example1.ws_contigset_id
...
# Authentication: required
	...
	method => "example1.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for example1::example1Client\n";
    }
    if ($sMajor == 0) {
        warn "example1::example1Client version is $svr_version. API subject to change.\n";
    }
}
...
package example1::example1Client::RpcClient;
